...
export function AdminCombos() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [comboRes, productRes, categoryRes] = await Promise.all([
        fetch(`${env.API_URL}/combos`),
  ...
  const openEdit = (combo: any) => {
    ...
    setEditing(combo);
    setForm({ categoryId: combo.categoryId, name: combo.name, description: combo.description || "", imageUrl: combo.imageUrl || "", sku: variant?.sku || "", price: String(variant?.price ?? ""), isActive: combo.isActive });
  ...
  const save = async () => {
    if (!form.name.trim() || !form.categoryId || !form.sku.trim() || form.price === "") return toast.error("Vui lòng nhập đủ tên, danh mục, SKU và giá combo.");
  ...
  return <div className="space-y-5">
    ...
    {showModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={() => setShowModal(false)}><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-sidebar p-6 shadow-2xl" onClick={event => event.stopPropagation()}>
      ...
      <div className="grid gap-3 sm:grid-cols-2"><input className="rounded-xl bg-sidebar-accent px-3 py-2 text-sm outline-none" placeholder="Tên combo" value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} /><select className="rounded-xl bg-sidebar-accent px-3 py-2 text-sm outline-none" value={form.categoryId} onChange={event => setForm({ ...form, categoryId: event.target.value })}><option value="">-- Chọn danh mục --</option>{categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select><input className="rounded-xl bg-sidebar-accent px-3 py-2 text-sm outline-none" placeholder="SKU combo" value={form.sku} onChange={event => setForm({ ...form, sku: event.target.value })} /><input type="number" min="0" step="1000" className="rounded-xl bg-sidebar-accent px-3 py-2 text-sm font-semibold text-primary outline-none" placeholder="Giá combo" value={form.price} onChange={event => setForm({ ...form, price: event.target.value })} /><textarea className="rounded-xl bg-sidebar-accent px-3 py-2 text-sm outline-none sm:col-span-2" rows={2} placeholder="Mô tả combo" value={form.description} onChange={event => setForm({ ...form, description: event.target.value })} /><div className="sm:col-span-2"><ImageUploader label="Hình ảnh combo" value={form.imageUrl} onChange={imageUrl => setForm({ ...form, imageUrl })} /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isActive} onChange={event => setForm({ ...form, isActive: event.target.checked })} />Hiển thị combo</label></div>
