...
export class ProductsService {
    ...
    async deleteCategory(id: string): Promise<void> {
        ...
        const productsCount = await this.productsRepository.count({ where: { categoryId: id } });
        if (productsCount > 0) {
    ...
    async createVariant(productId: string, dto: CreateProductVariantDto): Promise<ProductVariant> {
        ...
        const existingSku = await this.variantsRepository.findOne({ where: { sku: dto.sku } });
    ...
    async deleteVariant(id: string): Promise<void> {
        ...
        const count = await this.variantsRepository.count({ where: { productId: variant.productId } });
        if (count <= 1) {
